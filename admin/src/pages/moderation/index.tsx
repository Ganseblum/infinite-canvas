...
import { fetchModerationPreviewUrl } from "@admin/services/api/moderation-preview";
...
export default function AdminModerationPage() {
    ...
    // 隔离预览要先带令牌取回再转 blob URL，不能用 previewPath 直接当 src（见 moderation-preview.ts）。
    // previewPath 变化（换记录、关闭面板）或组件卸载时释放上一张 object URL。
    const previewPath = detailQuery.data?.record.quarantine?.available ? detailQuery.data.record.quarantine.previewPath : undefined;
    ...
    useEffect(() => {
        ...
        fetchModerationPreviewUrl(previewPath, controller.signal)
            .then((url) => {
                objectUrl = url;
                setPreviewUrl(url);
            })
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {recordsQuery.isError ? (
                <QueryError error={recordsQuery.error} message={t("admin.moderation.loadFailed")} onRetry={() => void recordsQuery.refetch()} />
            ) : (
            ...
            <Drawer open={!!selectedId} width={640} title={t("admin.moderation.detailTitle")} onClose={() => setSelectedId(null)}>
                {detailQuery.isError ? (
                    <QueryError error={detailQuery.error} message={t("admin.moderation.detailFailed")} />
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        {detailQuery.data.record.quarantine?.available ? (
                            <div className="flex flex-col gap-2">
                                ...
                                {previewFailed ? (
                                    <Alert type="warning" showIcon message={t("previewFailed", { ns: "admin" })} />
                                ) : previewUrl ? (
                                    <img src={previewUrl} alt={t("admin.moderation.previewAlt")} className="max-h-72 w-full rounded-lg border border-stone-200 object-contain dark:border-stone-800" />
                                ) : (
                                    <div className="py-6 text-center text-sm text-stone-500 dark:text-stone-400">{t("admin.loading")}</div>
                                )}
                            </div>
