...
## 终端用户账号统一（SSO）：注册一次，全产品可用

背景问题：用户在优刻画布注册后，去 blog / 办公套件还要再注册一次吗？答案是**规划做统一账号，但不是共享 Cookie**。触发条件同样是第二个产品立项；本节先把设计定下来，避免各产品各建一套用户表。

### 结论

1. **身份统一、业务隔离**：建一个平台账号中心（IdP，如 `auth.youc.online` 或主站 `/api/auth` 扩展为 OIDC Provider），持有密码、邮箱验证、找回密码与注销；各产品后端只保存「本地档案」——首次用优刻账号登录时通过 userinfo 自动创建/关联（以平台用户 ID 为唯一标识），画布点数、blog 文章这类业务数据仍留在各产品自己的库。
2. **接入方式用 OIDC 授权码模式**，不是共享 Cookie：产品登录页放「使用优刻账号登录」，跳账号中心授权后带 code 回来换令牌。**明确延续既有红线：不把 refresh cookie 提到根域名 `.youc.online`**——共享根域 Cookie 意味着任何子域的 XSS 都能拿到全站身份，且各产品会话无法独立吊销。
3. **SSO 体验**来自「账号中心已登录 → 一键授权」，而不是浏览器层面共享登录态：在每个产品点一次授权即可，之后该产品保持自己的会话。
4. **点数口径需要拍板（产品决策）**：
   - 方案 A 平台钱包：点数是「优刻平台货币」，账本在账号中心，各产品扣费时调中心 API（扣费流水带 product 维度）。优点是一个余额处处可用、充值一次全产品受益；缺点是账本成为全平台单点，对账与退款链路都收口到中心。
   - 方案 B 各产品钱包：blog 免费或单独计费，与画布点数无关。优点是隔离简单；缺点是用户要分别充值。
   - 建议：若办公套件与 blog 走免费或订阅制，先用方案 B（零改造）；一旦第二个产品也要按量扣点，直接上方案 A，不要再造一套账本。
5. **合规联动**：统一账号后，注销与数据导出必须覆盖全部产品——账号中心的注销要向各产品广播删除（或各产品订阅账号删除事件），隐私导出要能聚合各产品数据。这在方案设计时就要进契约，不能事后补。

### 各产品接入的改动清单（立项时执行）

1. 账号中心：主站 auth 模块扩展 OIDC Provider（authorize/token/userinfo/jwks 四个端点），注册页文案改为「一个优刻账号，畅用全部产品」。
2. 各产品后端：接入 OIDC 登录回调 + 本地档案表（platform_user_id 唯一索引），保留邮箱密码登录作为降级入口（可选）。
3. 各产品管理后台：`/admin/meta` 的 product.id 与账号中心的 client_id 对应；用户管理页展示「平台身份 + 本地档案」两列。
4. 注销与导出：账号中心提供用户删除 webhook，各产品实现并保证幂等。

### 与 admin 多产品的关系

admin 的统一（已落地）管的是「运营者」；本节管的是「终端用户」。两者独立：admin 会话仍按产品隔离，不做平台级 SSO（P3 维持原判）；终端用户 SSO 在第二个产品立项时与 P1 的 per-product apiBase 切换一起实施。
