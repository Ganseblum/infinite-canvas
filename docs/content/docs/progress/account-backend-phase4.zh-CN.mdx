---
title: 第四期实施清单：AI 请求后端转发与点数计量
description: 账号体系与后端服务规划第四期的详细开发底稿，涵盖平台模型的后端转发、用户渠道保持浏览器直连、流式透传、扣点一致性与平台密钥托管
---

# 第四期实施清单：AI 请求后端转发与点数计量

本文档是[账号体系与后端服务规划](/zh-CN/docs/progress/account-backend-plan)第四期的开发底稿。前置条件是[第三期](/zh-CN/docs/progress/account-backend-phase3)已交付并通过验收，`model_catalog` 与点数流水已经可用。
...
支付与点数充值。点数的发放仍然只能通过第三期的管理后台手工调整，接入支付渠道不在规划范围内。
...
第三期引入了点数收费，但点数目前扣不准，因为生成行为压根不经过本项目的服务端。第三期的文档已经把这件事写成了「软约束」：浏览器直连用户自己配置的第三方渠道，服务端只能依赖前端在生成完成后主动上报一条生成记录来计数，改几行前端代码就能绕过。当计数只用于「每天最多生成 50 张」这类礼貌性限制时，软约束是可接受的；一旦点数变成要花钱买的东西，软约束就等于卖一个无法计量的商品——用户付了钱，我们不知道他用了多少；用户没付钱，我们也拦不住他用。
...
**有一件事本期做不到，需要提前说清楚：第二期留下的凭据明文下发不会消失。** 第二期的 `GET /api/credentials` 返回的是**完整明文** API Key，理由是前端必须拿着它直连第三方渠道；本期用户渠道继续直连，这个理由依然成立，所以这个接口继续返回明文，第二期文档里「第四期会关掉它」的说法在本期不兑现。它的直接后果是任何拿到用户 access token 的人仍然能读走该用户全部厂商密钥，因此第一期定下的「access token 只存内存、refresh token 走 httpOnly cookie」这条防线必须长期维持，不能因为平台模型有了后端转发就放松。本期真正消掉的暴露面只有一个：平台自己的渠道 Key 从头到尾不进浏览器。
...
| 402 | `INSUFFICIENT_CREDITS` | 点数不足，`error` 里带 `required` 与 `balance` |
...
`EMAIL_NOT_VERIFIED` 与 `STORAGE_QUOTA_EXCEEDED` 都必须**在扣点之前**判定。生成产物一定要落到媒体存储，如果等生成完了才发现存不下，用户既拿不到结果又白扣一次点数，退款流程再正确也是糟糕的体验。

### 请求与响应的通用形状

五个接口全部需要登录，全部挂在 `/api/ai` 下，全部由第一期的 `RequireAuth` 中间件保护，并且**只服务平台目录里的模型**。

请求体统一带 `model` 字段，其余字段按能力各自定义。可选地带一个 `idempotencyKey`（前端用 `nanoid` 生成），用于避免重试和双击造成的重复扣点，详见「并发与重复提交」。

响应统一包含两段信息：`credits` 给出本次消耗与最新余额，产物一律以 `storageKey` 形式给出。**不透传上游的原始响应体。** 平台侧的上游同样五花八门，`parseImagePayload` 现在要兼容 `data`、`images`、`results` 三种字段名，`resolveImageDataUrl` 要同时处理 `b64_json` 和 `url`，`unwrapEnvelope` 还要剥一层 `{code, data, msg}` 信封——平台路径上的这些兼容分支收敛到服务端一处，前端对 `/api/ai/*` 只认一种形状。
...
请求 `{ "model": "...", "input": "要朗读的文本", "voice": "alloy", "format": "mp3", "speed": 1, "instructions": "..." }`，字段与现有 `requestAudioGeneration` 发给 `/audio/speech` 的请求体一一对应，只是 `response_format` 改名为 `format`、`input` 保持不变。
...
校验必须发生在扣点和调用上游**之前**。顺序固定为：鉴权 → 模型标识校验 → 邮箱验证与存储配额预检 → 参数校验 → 并发槽位占用 → 扣点 → 调用上游。任何一步失败都直接返回，不进入下一步。这个顺序不是随意排的：把最便宜、最确定的检查放在前面，才能保证一个明显非法的请求既不消耗上游额度也不消耗用户点数。
...
扣点在一个数据库事务里完成：校验余额、扣减余额、写一条消费流水。事务提交之后才发起上游请求。余额不足时返回 402 `INSUFFICIENT_CREDITS` 并带上 `required` 与 `balance`，前端据此提示还差多少。

退还写一条 `type=refund` 流水（第三期的点数流水表，下文按 `credit_ledger` 指代，实际表名以第三期为准），金额与被退的消费流水相等，并且**在流水里记 `ref_id` 指向那条消费记录**，同时对 `(ref_id, type)` 建唯一索引。这个唯一索引是幂等的关键：重试、并发、进程重启导致退还逻辑被执行两次时，第二次会因为唯一冲突而失败，用户不会拿到双倍退款。
...
| 路径 | 职责 |
| --- | --- |
| `internal/handler/ai.go` | 五个接口的入口：模型标识校验、参数校验、配额与邮箱预检、扣点、调用 provider、结果落地 |
...
| `internal/service/credit.go` | 扣点、退还、幂等判定，五个接口共用 |
...
type AIRequest struct {
    ...
    ChannelID      *uuid.UUID `gorm:"type:uuid"` // 实际使用的 PlatformChannel
...
索引：`ai_requests` 上唯一索引 `(user_id, idempotency_key)`（允许 `idempotency_key` 为空，用部分索引跳过空值），另建 `(user_id, created_at desc)` 服务管理后台查询；`ai_tasks` 上 `(status, updated_at)` 服务启动时的恢复扫描，以及 `(user_id, id)` 服务归属校验。
...
导出 `generateImages`、`createVideoTask`、`getVideoTask`、`generateSpeech`、`streamChat` 五个函数，对应五个接口，**只在平台模型分支被调用**。前四个直接走第一期的 `web/src/services/api/client.ts`，自动获得 token 注入、静默刷新和 `error.code` 的类型化异常。
...
三个细节容易踩。`proxy_pass` 结尾的路径要与第一期 `location /api/` 的 `proxy_pass http://api:8080/;` 保持同样的改写规则（都把 `/api` 前缀剥掉），写错了就是全线 404。`proxy_http_version 1.1` 加上清空 `Connection` 头是分块传输的前提，默认的 HTTP/1.0 不支持分块，SSE 会被整段缓冲。`client_max_body_size` 可以比第一期媒体上传的 200m 小很多，因为参考图现在传的是 `storageKey` 而不是 base64。
...
- 点数不足时返回 402 `INSUFFICIENT_CREDITS`，`error` 里带 `required` 与 `balance`，且**没有发起任何上游请求**。
...
- 未验证邮箱调用生成接口，在扣点之前就返回 403 `EMAIL_NOT_VERIFIED`。
