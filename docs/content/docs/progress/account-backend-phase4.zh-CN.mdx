...
幂等由数据库兜底，但索引必须是**部分唯一索引** `(type, ref_id) WHERE type = 'refund'`，不能对 `(ref_id, type)` 无条件建唯一。第三期管理员手工增减点数写的是 `type=grant`、`ref_id` 为操作管理员 id，无条件的唯一索引会让同一个管理员发第二次点就撞上冲突被拒。只约束 refund 之后，这个索引仍然是退还幂等的关键：重试、并发、进程重启导致退还逻辑被执行两次时，第二次会因为唯一冲突而失败，用户不会拿到双倍退款。
