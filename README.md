...
- `upstream/main` 只作为源头代码，不直接部署；账号体系与服务端规划暂放在 `codex/account-backend-plan` 分支的 `我的规划/` 目录。
- 定期同步源头时先更新 `main`，再把 `main` 合并到规划或功能分支。完整流程见[**Fork 同步、分支与部署约定**](我的规划/Fork同步与部署约定.md)。
