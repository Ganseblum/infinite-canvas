...
| T04 | 【M1】真实 MySQL 迁移验收 + 全量回归：go test ./... 全绿；真实 MySQL 起服务完成迁移一次（字符串列长度/保留字列名红线）；五个回归点（登录/充值/生成扣费/配额 507/注销）过一遍；web/ 零 diff 机器核对；评审门① 在本任务前 | P0 | server/cmd/server/main.go、server/internal/db/db.go | T03 | ◐ 进行中 | 评审门① 通过（P1/P2 已修复）；SQLite 全量回归绿；web/ 零 diff 通过；真实 MySQL 迁移验收执行中（口径：测试库重建而非原地升库，未上线无数据搬迁） |
