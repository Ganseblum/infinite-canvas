...
进度：11/11 已完成、0 进行中、0 任务阻塞；范围 = M1 身份切换 + M2 权益中心化 + M3 OIDC Provider（画布前端零改动红线贯穿全程），全部落地并部署测试环境。D1-D8 已拍板关闭，D9（对账告警通道）保持未决。评审门① 通过（2026-09-18，P1/P2 已修复）；评审门②③④ 统一复核通过（2026-09-19，独立 reviewer）：P1×2 已修复（误入库的 48MB 构建产物已移除并 ignore；订阅作废/自然到期的存储配额同步已在作废事务与夜间对账任务双保险落地），P2×7 已修复 3（SSO 更新审计补 after、只读测试名 403 纠偏、OIDC_JWKS_PRIVATE_KEY 进 env 模板）、4 项入 todo（token 限流、authorize 浏览器承接、会员订单回调用例、deletion 多产品记账）。测试环境部署 + 浏览器自动化验收通过（登录/档位派生/配额同步/SSO CRUD/F5 保持登录/非管理员后台门），验收截图存档 gui-test-screenshots/。
...
| --- | --- | --- | --- | --- | --- | --- |
| T01 | 【M1】platform_users 落表与模型改造：users→platform_users（主键值不变）、refresh_tokens→sessions，业务表列名不改（user_id 语义即平台 id）；同步 model 定义与 db.Migrate AutoMigrate 清单 | P0 | server/internal/model/model.go、server/internal/db/db.go | — | ✅ 已完成 | 提交 dce9e9c：model.PlatformUser / model.Session 落表（GORM 默认命名即 platform_users / sessions），AutoMigrate 切换，全仓旧名零残留；字段与列名全部不变 |
| T02 | 【M1】接口奇偶清单（parity inventory，先于切换）：从真实源码枚举 /api/auth/*、/api/me/*、/api/media/*、/api/credits*、/api/orders、/api/plans 及 admin 相关端点的请求/响应键、错误码与 body，逐项决定 replicate / omit（写明理由）/ defer；清单作为 T03/T06 的对照基准 | P0 | server/internal/handler/（auth.go、account.go、media.go、order.go、admin.go） | — | ✅ 已完成 | records/plans/PLAN-PLATFORM-ACCOUNT-MEMBERSHIP-parity-inventory.md：118 端点全 replicate / 0 omit / 0 defer；断裂点核实 5 处（新增 community.go:366、activity.go:169 两处裸列投影——列名 M1 不变故安全）；核实 READ_ONLY 实际 403 非 402，M1 按现状执行，M2 如改 402 属用户可见变更需单独确认 |
| T03 | 【M1】auth/middleware/admin/authz 全量切换到 identity 域：/api/auth/*、/api/me/* 键不变、值来源换域服务；清裸 SQL 断裂点（handler/admin.go:274、middleware/authz.go:66，grep 逐一确认无遗漏）；testutil barrier 表名 refresh_tokens→sessions | P0 | server/internal/handler/auth.go、server/internal/middleware/、server/internal/handler/admin.go、server/internal/handler/testutil_test.go | T01, T02 | ✅ 已完成 | 提交 dce9e9c：identity 域唯一入口（auth/account/admin×4/middleware×5），裸 SQL 断裂点清零（grep 复核 Table("users")/Table("refresh_tokens") 为 0），testutil barrier 改 sessions；go test 14 包全绿；web/、admin/ 零 diff 机器核对通过 |
| T04 | 【M1】真实 MySQL 迁移验收 + 全量回归：go test ./... 全绿；真实 MySQL 起服务完成迁移一次（字符串列长度/保留字列名红线）；五个回归点（登录/充值/生成扣费/配额 507/注销）过一遍；web/ 零 diff 机器核对；评审门① 在本任务前 | P0 | server/cmd/server/main.go、server/internal/db/db.go | T03 | ✅ 已完成 | go test 14 包全绿；评审门① 通过（P1/P2 已修复，提交 668336b）；web/ 零 diff 机器核对通过；真实 MySQL 8.4.11 迁移验收通过（测试服务器一次性 scratch 库：AutoMigrate 32 表 298 列无错、platform_users/sessions 落表、EnsureAdmin 成功、登录 API 冒烟通过，验后即删）；口径为新建库而非原地升库（未上线无数据搬迁） |
| T05 | 【M2】三域建表与域服务：membership_plans（seed 保留 free/paid/sunset id）+ membership_subscriptions；credit_accounts 替换 credits（移除 paid_until，付费身份由 subscription.period_end 表达）；credit_transactions/orders/media_files 加 product varchar(32) default 'youc-canvas'；credit_packages 按两张表拆分；storage_accounts/storage_usage 新表；free_grant_claims 归 billing；usage_records 仅留试用计数（storage_bytes 指标退役）；plans 表 M2 退役；四域接口按契约表签名落地；零物理外键延续 | P0 | server/internal/platform/{identity,membership,billing,storage}/、server/internal/model/、server/internal/db/db.go | T04 | ✅ 已完成 | 提交 684baa6：model/platform.go 四新表（MembershipPlan/MembershipSubscription/StorageAccount/StorageUsage，StorageUsage 显式 TableName 契约名）、credit_accounts 替换 credits（PaidUntil 移除）、product 维度加齐、credit_packages 拆分、plans 表退役、cursor 抽包；三域服务测试齐备 |
| T06 | 【M2】画布业务切换：档位派生 purchased>0⇒paid 废除，改为 membership.ActivePlan（只有点数无订阅回落 free 档但可消费；graceEndsAt=period_end+60 天），映射对照表随本任务交付；生成扣费走 billing.Reserve/Refund；配额预检/复核走 storage.Check/Commit（507/402 body 不变）；试用走 ConsumeFreeTrial/FreeTrialRemaining；跨域写由编排层传 tx 单事务；评审门② 在本任务前 | P0 | server/internal/service/{credit.go,quota.go,free_grant.go,media_write.go,orders.go}、server/internal/handler/{ai.go,media.go} | T05 | ✅ 已完成 | 提交 684baa6：credit.go 删除、quota.go 瘦身；生成扣费预扣/试用占用与 ai_requests 行单事务（重复幂等键整体回滚，外部行为不变）；档位派生废除改 ActivePlan（D6）；paidUntil 键保留值改订阅 period_end；/api/orders 支持 planId；403 READ_ONLY/507 body 逐字节保留；web/ 零 diff 复核通过；映射对照=口径决定记录（paidUntil 来源、ListUsers 新派生、ListPackages entitlementDays=0、ListPlans 读 membership_plans） |
| T07 | 【M2】admin 加法扩展：/api/admin/users 响应追加 products/membership/storage 字段（纯加法）；新增会员管理模块（订阅查询/权益调整/补偿）；authz 目录增 membership.read/write 权限点；流水与用量页增 product 筛选；/admin/meta 形状不变 | P1 | server/internal/handler/admin.go、server/internal/authz/catalog.go、admin/src/ | T05 | ✅ 已完成 | 提交 57947cc（前端：会员管理页 / users 会员存储列 / i18n）+ a93d3e2（后端 admin_membership.go 四端点）+ 509834a（authz 权限点）；users 响应纯加法，/admin/meta 形状不变 |
| T08 | 【M2】关键路径异常矩阵落地验证 + 夜间对账任务：生成扣费/支付回调/媒体落盘记账三条路径按异常矩阵逐行验证并留证据；夜间对账任务比对 media_files.bytes 聚合 vs storage_usage vs storage_accounts，不平走 storage.Recalculate 修正 + 告警（告警通道见未决项 D9）；评审门③ 在本任务前 | P0 | server/internal/service/cleanup.go（或新增对账任务文件）、server/internal/platform/storage/ | T06, T07 | ✅ 已完成 | 提交 8f870bc + a93d3e2：service/reconcile.go 三层比对（L1 media_files / L2 storage_usage / L3 storage_accounts）不平走 Recalculate 收敛，单测覆盖两处漂移；夜间任务接入 main.go（24h）；告警先落结构化日志 storage_reconcile_drift，D9 拍板后接线；三条路径幂等前置由唯一键断言与域测试锁死（ai_requests unique、provider_order_id unique、media_files 行记账） |
| T09 | 【M3】OIDC Provider 四端点 + oauth_clients：GET /api/oidc/authorize（支持 PKCE；ic_refresh cookie Path 从 /api/auth 扩到 /api，仍 host-only+Lax+HttpOnly）、POST /api/oidc/token（RS256 15min）、GET /api/oidc/userinfo、GET /api/oidc/jwks.json；oauth_clients 表（product 与 /admin/meta product.id 同值）；authz 增 sso.read/write | P1 | server/internal/handler/oidc.go（新增）、server/internal/model/、server/internal/authz/catalog.go、server/internal/handler/auth.go | T06 | ✅ 已完成 | 提交 a93d3e2：authorize（PKCE 强制 S256、redirect_uri 精确匹配、code 一次性进程内存储）/ token（RS256 15min、恒时比对）/ userinfo / jwks 四端点 + oauth_clients 表；ic_refresh Path /api/auth→/api（D8 直接切换，host-only+Lax+HttpOnly 不变）；OIDC_JWKS_PRIVATE_KEY fail-fast；集成测试 9 例含安全用例全绿 |
| T10 | 【M3】admin SSO 管理页 + 集成测试：admin 新增 OAuth client 管理页（sso.read/write）；集成测试用测试 client（client_id/secret/redirect_uri/PKCE）跑通 authorize→token→userinfo→jwks 全链路，含未注册 redirect_uri 拒绝、PKCE 缺失拒绝、token 过期用例；评审门④ 在本任务前 | P1 | admin/src/、server/internal/handler/（oidc 集成测试） | T09 | ✅ 已完成 | 提交 5597725（前端 SSO 管理页：列表/新增/编辑/重置密钥/删除，secret 一次性展示）+ a93d3e2（后端 admin_sso.go 五端点 CRUD + 回调 https 校验 + 审计 + 路由计数 60）；全链路与安全用例在 T09 集成测试全绿 |
| T11 | 【横切】测试设计（test-planner）：产出用例库 records/tests/infinite-canvas-platform-account-membership/（契约用例对照 T02 奇偶清单、五个回归点、三条关键路径异常矩阵、OIDC 安全用例）；冒烟集供「验证命令」引用；端到端用例须在评审门③/④ 关闭前全部通过 | P1 | records/tests/infinite-canvas-platform-account-membership/ | T02 | ✅ 已完成 | 提交 f6b1401：148 条用例（契约 94 / 回归 28 / 异常矩阵 12 / OIDC 14）+ 74 个冒烟测试函数索引；记录文档-代码不一致 5 处（含 READ_ONLY 403 口径） |
...
| D8 | ic_refresh Cookie Path 扩面顺序 | M3 直接将 Path 从 /api/auth 扩为 /api，是否需要旧 Path 双发兼容期及灰度顺序 | 待 T09 设计评审定，倾向直接切换（未上线无兼容包袱） | T09 | ✅ 已关闭（T09 直接切换，无双发兼容期，host-only+Lax+HttpOnly 不变） | 技术负责人 |
