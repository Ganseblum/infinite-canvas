...
+ [新增] 开发文档新增项目结构说明，覆盖仓库目录划分、各子项目技术栈、核心模块与数据流。
+ [新增] 进度文档新增账号体系与后端服务中文总规划及五期执行计划，逐期明确实施顺序、偏离纠正和验收门禁，并补充参数价格矩阵、模型及参数级限时折扣、促销生命周期与审计、生成前报价预扣、计费快照和内容审核人工复核。
+ [调整] 账号后端规划明确改用 MySQL 8.0、可配置 SMTP（当前以 QQ 邮箱授权码作为小体量部署示例），并增加邮箱验证后的免费赠送领取记录与多维防刷规则。
