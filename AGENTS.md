...
- **顺序不能反：先确认分支，再动手改。** 编辑文件、`git add` / `git commit` / `git merge` / `git rebase` / `git push` 之前，先运行 `git branch --show-current`（或 `git rev-parse --abbrev-ref HEAD`）确认当前分支，确认无误后才开始改动。
- **禁止「先改后查」**：不要等改完、提交前才检查分支——那时改动已经落在错误的分支上了。一旦发现自己先改后查，立即停下并报告现状，不要继续提交或推送。
- 当前分支与本次任务预期不符时，停下来向用户确认，不要按猜测继续。
- 分支职责与命名按 `我的规划/Fork同步与部署约定.md`：新建分支一律按用途选前缀——`codex/<主题>`（主线规划与实现）、`feature-<主题>`（单个功能/一期）、`fix-<主题>`（缺陷修复）、`sync/upstream-<主题>`（上游同步）、`release_sim_<主题>` / `release_pre_<主题>` / `release_prod_<主题>`（部署与解冲突）；全小写、连字符分隔、主题用英文短词，一个分支只做一件事，合并后删除。不直接在 `main` 上开发，正式环境只从经过验收的 `main` 提交或版本 tag 部署。
- 分支若命中 `release_sim_*` / `release_pre_*` / `release_prod_*`，任何写操作都要先说明「当前分支 + 准备改什么」并取得当次确认；唯一例外是解决该分支上的合并冲突，且不得夹带其它改动。
- **合并与集成必须取得明确授权，不许自动做。** 未经用户明确要求，不得执行 `git merge`、`git pull`（非 `--rebase` 的合并式拉取）、把 `main` / `upstream/main` 或任何分支合并进当前分支、跨分支 `git cherry-pick`，也不得为了「顺便同步一下」自行集成其它分支。需要集成时先说明「要合并哪个分支、目标分支是什么、为什么必须合并、预期影响」，得到当次确认后再执行。
- **推送被拒需要先集成远程改动时，同样先问。** 先只做只读的 `git fetch` / `git log` / `git diff` 把「远程多了哪些提交、改了哪些文件、与本地的重叠在哪」讲清楚，再让用户选择 `merge` 还是 `rebase`；不要自行选一种方式直接把远程改动合成进来。
- 一次任务中只要切换过分支（合并、切分支排查、切换工作分支），后续每次写操作都要重新确认。
...
- 响应式布局统一按三档断点约定：移动 <768（Tailwind 无前缀默认态，显隐用 `md:hidden`）、平板 768–1023（`md:`，两列起步）、桌面 ≥1024（`lg:` / `xl:`，完整多栏）；布局形态切换点只允许落在 md（768）与 lg（1024）两处，不各自发明断点。手写 CSS media 统一用 `(max-width: 767.98px)`；antd Modal/Drawer 的移动端形态由 `web/src/styles/globals.css` 的全局规则统一钳制（admin 经 @import 同步生效），不要在业务组件里为单个弹窗写私有 media 覆盖。
...
- SQLite 单测覆盖不到 MySQL 特有错误：字符串列长度、MySQL 保留字列名都已两次在真库才炸（11 个测试包全绿而真库迁移/启动失败）。涉及新表或新列名的改动，必须在一台真实 MySQL 上跑一次迁移再交付。
- 环境变量的 fail-fast 不等于模板真的读了它：新增运行期注入变量时，必须沿「入口脚本注入 → index.html 加载 /config.js → 代码读取」全链路核对，只检查注入不等于生效。
