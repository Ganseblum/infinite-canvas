...
func createCanvas(t *testing.T, r http.Handler, token string, body map[string]any) map[string]any {
	t.Helper()
	w := doAuthJSON(r, http.MethodPost, "/api/canvases", token, body)
...
func TestCanvasCRUDRevisionAndSoftDelete(t *testing.T) {
	...
	user := createUser(t, g, "canvas@example.com", "canvasuser", "password123", true)
	...
	// 列表只回摘要，不含 data
	w := doAuthJSON(r, http.MethodGet, "/api/canvases", token, nil)
	...
	// 详情含 data 与 revision
	w = doAuthJSON(r, http.MethodGet, "/api/canvases/"+id, token, nil)
	detail := decodeBody(t, w)
	...
	w = doAuthJSON(r, http.MethodPut, "/api/canvases/"+id, token, map[string]any{"data": newData, "revision": 1})
	if w.Code != http.StatusOK {
		t.Fatalf("PUT 失败: code=%d body=%s", w.Code, w.Body.String())
	}
	if decodeBody(t, w)["revision"].(float64) != 2 {
	...
	w = doAuthJSON(r, http.MethodPatch, "/api/canvases/"+id, token, map[string]any{"title": "新标题"})
	if w.Code != http.StatusOK {
		t.Fatalf("PATCH 失败: code=%d body=%s", w.Code, w.Body.String())
	}
	patchBody := decodeBody(t, w)
	...
	// 删除：软删、幂等、删除后 404
	if w := doAuthJSON(r, http.MethodDelete, "/api/canvases/"+id, token, nil); w.Code != http.StatusNoContent {
		t.Fatalf("删除应返回 204, got %d", w.Code)
	}
	if w := doAuthJSON(r, http.MethodGet, "/api/canvases/"+id, token, nil); w.Code != http.StatusNotFound {
...
func TestCanvasCrossUserIsolation(t *testing.T) {
	...
	cfg := testConfig()
	r := newResourceRouter(t, g, cfg, newFakeStorage("local"))
	owner := createUser(t, g, "owner@example.com", "owneruser", "password123", true)
	other := createUser(t, g, "other@example.com", "otheruser", "password123", true)
	...
	for _, tc := range []struct {
		...
		w := doAuthJSON(r, tc.method, "/api/canvases/"+id, otherToken, tc.body)
	...
	// 原用户的记录未被影响
	if w := doAuthJSON(r, http.MethodGet, "/api/canvases/"+id, ownerToken, nil); w.Code != http.StatusOK {
...
func TestCanvasListPaginationSearchAndSortValidation(t *testing.T) {
	...
	cfg := testConfig()
	r := newResourceRouter(t, g, cfg, newFakeStorage("local"))
	user := createUser(t, g, "list@example.com", "listuser", "password123", true)
	token := accessToken(t, cfg, &user)
	...
	// 分页
	w := doAuthJSON(r, http.MethodGet, "/api/canvases?page=1&size=2", token, nil)
	body := decodeBody(t, w)
	if body["total"].(float64) != 3 || body["page"].(float64) != 1 || body["size"].(float64) != 2 {
		t.Fatalf("分页响应不符: %v", body)
	}
	if len(decodeItems(t, w)) != 2 {
		t.Fatalf("第一页应返回 2 条")
	}
	w = doAuthJSON(r, http.MethodGet, "/api/canvases?page=2&size=2", token, nil)
	if items := decodeItems(t, w); len(items) != 1 {
		t.Fatalf("第二页应返回 1 条, got %d", len(items))
	}

	// 搜索大小写不敏感
	w = doAuthJSON(r, http.MethodGet, "/api/canvases?q=alpha", token, nil)
	if items := decodeItems(t, w); len(items) != 1 || items[0]["title"] != "Alpha 草图" {
		t.Fatalf("搜索应大小写不敏感命中: %v", items)
	}
	// 排序白名单：title 升序
	w = doAuthJSON(r, http.MethodGet, "/api/canvases?sort=title", token, nil)
	items := decodeItems(t, w)
	...
	for _, path := range []string{
		"/api/canvases?sort=id",
		"/api/canvases?size=101",
		"/api/canvases?size=0",
		"/api/canvases?page=0",
		"/api/canvases?page=abc",
	} {
...
func TestCanvasDataLimitAndServerComputedFields(t *testing.T) {
	...
	cfg := testConfig()
	r := newResourceRouter(t, g, cfg, newFakeStorage("local"))
	user := createUser(t, g, "limit@example.com", "limituser", "password123", true)
	token := accessToken(t, cfg, &user)
	...
	w := doAuthJSON(r, http.MethodPost, "/api/canvases", token, huge)
	if w.Code != http.StatusBadRequest {
	...
	// data 不是对象同样非法
	w = doAuthJSON(r, http.MethodPost, "/api/canvases", token, map[string]any{"title": "bad", "data": []int{1, 2}})
	if w.Code != http.StatusBadRequest {
		t.Fatalf("数组 data 应返回 400, got %d", w.Code)
	}

	// 空 data 缺省为空画布
	w = doAuthJSON(r, http.MethodPost, "/api/canvases", token, map[string]any{"title": "空画布"})
	if w.Code != http.StatusCreated {
		t.Fatalf("缺省 data 应可创建: %d body=%s", w.Code, w.Body.String())
	}
	empty := decodeBody(t, w)
...
func TestCanvasTitleLengthValidation(t *testing.T) {
	...
	cfg := testConfig()
	r := newResourceRouter(t, g, cfg, newFakeStorage("local"))
	user := createUser(t, g, "title@example.com", "titleuser", "password123", true)
	token := accessToken(t, cfg, &user)

	long := strings.Repeat("题", 201)
	w := doAuthJSON(r, http.MethodPost, "/api/canvases", token, map[string]any{"title": long})
	if w.Code != http.StatusBadRequest {
	...
	id := created["id"].(string)
	w = doAuthJSON(r, http.MethodPatch, "/api/canvases/"+id, token, map[string]any{"title": long})
	if w.Code != http.StatusBadRequest {
		t.Fatalf("PATCH 超长标题应返回 400, got %d", w.Code)
	}
	// PUT 缺少 revision → 400
	w = doAuthJSON(r, http.MethodPut, "/api/canvases/"+id, token, map[string]any{"data": map[string]any{}})
	if w.Code != http.StatusBadRequest {
...
func TestCanvasDeletedRecordNotVisibleInDetail(t *testing.T) {
	...
	cfg := testConfig()
	r := newResourceRouter(t, g, cfg, newFakeStorage("local"))
	user := createUser(t, g, "softdel@example.com", "softdel", "password123", true)
	token := accessToken(t, cfg, &user)
	...
	if w := doAuthJSON(r, http.MethodPatch, "/api/canvases/"+id, token, map[string]any{"title": "x"}); w.Code != http.StatusNotFound {
		t.Fatalf("已删记录 PATCH 应 404, got %d", w.Code)
	}
	if w := doAuthJSON(r, http.MethodPut, "/api/canvases/"+id, token, map[string]any{"data": map[string]any{}, "revision": 1}); w.Code != http.StatusNotFound {
