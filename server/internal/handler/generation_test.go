...
func TestGenerationCursorPagination(t *testing.T) {
	...
	w := doAuthJSON(r, http.MethodGet, "/api/generations?size=2", token, nil)
	...
	pages := 1
	for cursor != "" {
		w = doAuthJSON(r, http.MethodGet, "/api/generations?size=2&cursor="+cursor, token, nil)
		if w.Code != http.StatusOK {
	...
	// 时间倒序
	w = doAuthJSON(r, http.MethodGet, "/api/generations?size=1", token, nil)
	items = decodeItems(t, w)
...
func TestGenerationPendingReturnsAllWithoutPagination(t *testing.T) {
	...
	cfg := testConfig()
	r := newResourceRouter(t, g, cfg, newFakeStorage("local"))
	user := createUser(t, g, "pending@example.com", "pendinguser", "password123", true)
	token := accessToken(t, cfg, &user)
	...
	w := doAuthJSON(r, http.MethodGet, "/api/generations?status=pending", token, nil)
	if w.Code != http.StatusOK {
	...
	if items := decodeItems(t, w); len(items) != 4 {
		t.Fatalf("pending 应一次返回全部 4 条: %d", len(items))
	}
	// kind 与 status 组合
	w = doAuthJSON(r, http.MethodGet, "/api/generations?kind=video&status=pending", token, nil)
	if items := decodeItems(t, w); len(items) != 3 {
...
func TestGenerationPendingLimit(t *testing.T) {
	...
	cfg := testConfig()
	r := newResourceRouter(t, g, cfg, newFakeStorage("local"))
	user := createUser(t, g, "pendinglimit@example.com", "pendinglimit", "password123", true)
	token := accessToken(t, cfg, &user)
	...
	w := doAuthJSON(r, http.MethodGet, "/api/generations?status=pending", token, nil)
	if items := decodeItems(t, w); len(items) != pendingLimit {
...
func TestGenerationValidationAndCrossUser(t *testing.T) {
	...
	cfg := testConfig()
	r := newResourceRouter(t, g, cfg, newFakeStorage("local"))
	owner := createUser(t, g, "genowner@example.com", "genowner", "password123", true)
	other := createUser(t, g, "genother@example.com", "genother", "password123", true)
	ownerToken := accessToken(t, cfg, &owner)
	otherToken := accessToken(t, cfg, &other)

	// 非法枚举与游标
	for _, path := range []string{
		"/api/generations?kind=audio",
		"/api/generations?status=queued",
		"/api/generations?size=101",
		"/api/generations?cursor=not-base64!!",
		"/api/generations?cursor=" + encodeForTest("no-separator"),
		"/api/generations?cursor=" + encodeForTest("2026-09-10T12:00:00Z|not-a-uuid"),
	} {
		w := doAuthJSON(r, http.MethodGet, path, ownerToken, nil)
	...
	// 跨用户详情与删除 404
	if w := doAuthJSON(r, http.MethodGet, "/api/generations/"+gen.ID.String(), otherToken, nil); w.Code != http.StatusNotFound {
		t.Fatalf("跨用户详情应 404, got %d", w.Code)
	}
	if w := doAuthJSON(r, http.MethodDelete, "/api/generations/"+gen.ID.String(), otherToken, nil); w.Code != http.StatusNotFound {
		t.Fatalf("跨用户删除应 404, got %d", w.Code)
	}
	// 详情正常
	if w := doAuthJSON(r, http.MethodGet, "/api/generations/"+gen.ID.String(), ownerToken, nil); w.Code != http.StatusOK {
		t.Fatalf("本人详情应 200, got %d", w.Code)
	}
	// 删除幂等，删除后列表不含
	if w := doAuthJSON(r, http.MethodDelete, "/api/generations/"+gen.ID.String(), ownerToken, nil); w.Code != http.StatusNoContent {
		t.Fatalf("删除应 204, got %d", w.Code)
	}
	if w := doAuthJSON(r, http.MethodDelete, "/api/generations/"+gen.ID.String(), ownerToken, nil); w.Code != http.StatusNoContent {
		t.Fatalf("重复删除应 204, got %d", w.Code)
	}
	if w := doAuthJSON(r, http.MethodGet, "/api/generations/"+gen.ID.String(), ownerToken, nil); w.Code != http.StatusNotFound {
		t.Fatalf("删除后详情应 404, got %d", w.Code)
	}
	w := doAuthJSON(r, http.MethodGet, "/api/generations", ownerToken, nil)
...
func TestGenerationNoWriteEndpoints(t *testing.T) {
	...
	cfg := testConfig()
	r := newResourceRouter(t, g, cfg, newFakeStorage("local"))
	user := createUser(t, g, "genwrite@example.com", "genwrite", "password123", true)
	token := accessToken(t, cfg, &user)

	if w := doAuthJSON(r, http.MethodPost, "/api/generations", token, map[string]any{}); w.Code != http.StatusNotFound {
		t.Fatalf("不应存在 POST /api/generations, got %d", w.Code)
	}
	if w := doAuthJSON(r, http.MethodPatch, "/api/generations/"+uuid.NewString(), token, map[string]any{}); w.Code != http.StatusNotFound {
