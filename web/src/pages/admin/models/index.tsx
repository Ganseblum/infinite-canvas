        ...
        <div className="flex flex-col gap-4">
            ...
            {modelsQuery.isError ? (
                <Alert
                    type="error"
                    showIcon
                    message={t("admin.models.loadFailed")}
                    description={getApiErrorMessage(modelsQuery.error)}
                    action={<Button size="small" onClick={() => void modelsQuery.refetch()}>{t("admin.retry")}</Button>}
                />
            ) : (
                <Table<AdminModel>
                    rowKey="id"
    ...
    return (
        <Drawer open={!!model} width={720} title={t("admin.promotions.title", { name: model?.displayName ?? "" })} onClose={onClose}>
            ...
            {promotionsQuery.isError ? (
                <Alert className="mt-4" type="error" showIcon message={t("admin.promotions.loadFailed")} description={getApiErrorMessage(promotionsQuery.error)} />
            ) : (
